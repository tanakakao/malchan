...
def test_local_shap_uses_only_submitted_rows_without_replacing_cache(monkeypatch) -> None:
    """Only selected request rows should be explained and cached XAI must remain intact."""
    ...
    def fake_get_shap_values(predictor, frame):
        ...
        return values, base, object(), frame.copy()
    ...
    assert captured[0]["x"].tolist() == [2.0, 4.0]
