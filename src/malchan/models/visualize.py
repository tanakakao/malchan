...
def yy_plot_ml(
    ...
    if not cv:
        if model.models[target].task=="classification":
            ...
            #     model.predict(proba=True)[target+"_"+target_item],
            #     None,
            #     residual
            # )
        ...
        if model.models[target].task=="classification":
            # fig = _yy_plot(
            #     model.models[target].y,
            #     model.cv_preds[target]["train"][target+"_"+target_item],
            #     model.cv_preds[target]["test"][target+"_"+target_item],
            #     residual
            # )
            # tickvals = list(model.models[target].idx2item.keys())
            # ticktext = [model.models[target].idx2item[k] for k in tickvals]
            # fig.update_layout(
            #     xaxis=dict(
            #         tickvals=tickvals,      # 実際の値
            #         ticktext=ticktext # 表示ラベル
            #     )
            # )
            cm = confusion_matrix(model.models[target].y.astype(int), model.cv_preds[target][train_test][target].astype(int))
            labels = model.models[target].target_items
            fig = go.Figure(
                data=go.Heatmap(
                    z=cm,
            ...
            fig = _yy_plot(
                    model.models[target].y,
                    model.cv_preds[target]["train"],
                    model.cv_preds[target]["test"],
                    residual
            )         
    return fig
...
def show_pd_and_ice(
    ...
    # --- xticks を「順序付きカテゴリ」として扱うか判定 ---
    xticks_list = list(xticks)
    is_categorical = (np.asarray(xticks).dtype.kind in ("O", "U", "S"))  # object/str系ならカテゴリ扱い

    # カテゴリ軸にする場合は、Plotlyが扱いやすいように文字列化（重要）
    if is_categorical:
        xticks_list = [str(v) for v in xticks_list]

    # ICE
    a
    import streamlit as st
    st.write(xticks_list)
    if ice:
        for i in range(X_PD.shape[1]):
            fig.add_trace(
                go.Scatter(
    ...
    fig.add_trace(
        go.Scatter(
            ...
            mode="lines",
            line=dict(color="orange", width=2),
            name=f"Partial Dependence({target_col})",
        )
    )

    # Actual data
    # if (X is not None) and (y is not None):
    #     x_actual = X[target_col].values.ravel()
    #     if is_categorical:
    #         x_actual = np.array([str(v) for v in x_actual])
    ...
    #             mode="markers",
    #             marker_color="red",
    #             name=f"Actual Data({target_col})",
    #         )
    #     )

    # --- レイアウト / x軸設定 ---
    xaxis_kwargs = dict(title=target_col)
    if is_categorical:
        xaxis_kwargs.update(
            dict(
                type="category",
                categoryorder="array",
                categoryarray=xticks_list,  # ← これで順序が固定される
                tickmode="array",
                tickvals=xticks_list,       # 表示するtickも xticks に固定
                ticktext=xticks_list,
            )
        )
