...
function FigurePanel({ state, emptyText }) {
  if (state.loading) {
    return <p className="empty-state">visualizationでPlotly図を生成しています...</p>;
  }
...
export default function ExplainPage() {
  ...
  const [importanceView, setImportanceView] = useState("shap");
  ...
  useEffect(() => {
    ...
    api.xaiSummary(modelInfo.model_id)
      ...
      .catch((error) => {
        if (active) setSummaryError(error.message || String(error));
      })
      .finally(() => {
  ...
  const importanceOptions = useMemo(() => {
    ...
    return [...new Set([...methods, "beeswarm"])];
  }, [targetSummary]);
  ...
  useEffect(() => {
    if (!importanceOptions.length) return;
    if (!importanceOptions.includes(importanceView)) {
      setImportanceView(importanceOptions[0]);
    }
  }, [importanceOptions, importanceView]);
  ...
  useEffect(() => {
    if (pdMode !== "2d") return;
    const first = numericPdpFeatures.includes(feature)
      ? feature
      : numericPdpFeatures[0] || "";
  ...
  useEffect(() => {
    ...
    api.visualizationYy(modelInfo.model_id, xaiTarget)
      .then((response) => {
        if (active) setYyFigure({ response, loading: false, error: "" });
      })
      .catch((error) => {
        if (active) {
          setYyFigure({
  ...
  useEffect(() => {
    ...
    api.visualizationImportance(modelInfo.model_id, xaiTarget, {
      method: importanceView,
      combined: true,
      top_n: 20,
    })
      .then((response) => {
        if (active) setImportanceFigure({ response, loading: false, error: "" });
      })
      .catch((error) => {
        if (active) {
          setImportanceFigure({
  ...
  const selectedImportanceState = importanceView === "beeswarm"
    ? beeswarmFigure
    : importanceFigure;
  const selectedImportanceEmptyText = importanceView === "beeswarm"
    ? "XAI計算後にSHAP Beeswarmを表示します。"
    : "XAI計算後に特徴量重要度を表示します。";
      ...
      <article className="panel xai-panel xai-overview">
        ...
        <div className="form-grid xai-controls">
          <label>
            ...
            >
              {Object.keys(summary?.targets || {}).map((target) => (
      ...
      <div className="xai-results-stack">
        <article className="panel xai-result-panel">
          <div className="xai-result-head">
            <span>01</span>
            <div>
              <strong>Y-Y PLOT</strong>
              <h3>実測値と予測値</h3>
            </div>
          </div>
          <FigurePanel
            state={yyFigure}
            emptyText="登録モデルの診断図を表示できません。"
          />
        </article>
        ...
        <article className="panel xai-result-panel">
          <div className="xai-result-head">
            <span>02</span>
            <div>
              <strong>IMPORTANCE / BEESWARM</strong>
              <h3>特徴量重要度・SHAP Beeswarm</h3>
            </div>
          </div>
          <section className="xai-card">
            <div className="xai-card-head">
              <span>DISPLAYED PLOT</span>
              <div className="xai-card-controls">
                <label className="compact-select">
                  表示
                  <select
                    value={importanceView}
                    onChange={(event) => setImportanceView(event.target.value)}
                  >
                    {importanceOptions.map((value) => (
                      <option key={value} value={value}>
                        {IMPORTANCE_VIEW_LABELS[value] || value}
                      </option>
                    ))}
                  </select>
                </label>
                {importanceView === "beeswarm" && beeswarmOutputs.length > 1 && (
                  <label className="compact-select">
                    出力
                    <select
                      value={beeswarmOutput}
                      onChange={(event) => setBeeswarmOutput(event.target.value)}
                    >
                      {beeswarmOutputs.map((output) => (
            ...
            <FigurePanel
              state={selectedImportanceState}
              emptyText={selectedImportanceEmptyText}
            />
          </section>
        </article>
        ...
        <article className="panel xai-result-panel">
          <div className="xai-result-head">
            <span>03</span>
            <div>
              <strong>PARTIAL DEPENDENCE</strong>
              <h3>PD 1D / 2D</h3>
            </div>
          </div>
          <div className="pd-toolbar">
            <div className="pd-mode-toggle">
              <button
                className={pdMode === "1d" ? "active" : ""}
                onClick={() => setPdMode("1d")}
              >
                1D
              </button>
              <button
                className={pdMode === "2d" ? "active" : ""}
                onClick={() => setPdMode("2d")}
              >
                2D
              </button>
            </div>
            <label>
              ...
              >
                {(pdMode === "2d" ? numericPdpFeatures : pdpFeatures).map((column) => (
            ...
            {pdMode === "2d" && (
              <label>
                ...
                <select
                  value={secondFeature}
                  onChange={(event) => setSecondFeature(event.target.value)}
                >
                  {numericPdpFeatures
                    .filter((column) => column !== feature)
            ...
            {pdMode === "1d" && pdOutputs.length > 1 && (
              <label>
                ...
                <select
                  value={pdOutput}
                  onChange={(event) => setPdOutput(event.target.value)}
                >
            ...
            {pdMode === "2d" && pd2dOutputs.length > 1 && (
              <label>
                ...
                <select
                  value={pd2dOutput}
                  onChange={(event) => setPd2dOutput(event.target.value)}
                >
            ...
            {pdMode === "1d" && (
              <label className="switch-label">
                <input
                  type="checkbox"
                  checked={includeIce}
                  onChange={(event) => setIncludeIce(event.target.checked)}
                />
                <span />
                ICEを表示
              </label>
          ...
          <section className="xai-card pd-result-card">
            {pdMode === "1d" ? (
              <FigurePanel
                state={pdpFigure}
                emptyText="XAI計算後に1D PDを表示します。"
              />
            ) : (
              <FigurePanel
                state={pdp2dFigure}
                emptyText="2D PDには異なる2つの数値特徴量が必要です。"
              />
            )}
          </section>
        </article>
